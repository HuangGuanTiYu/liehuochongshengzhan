...
#define circleCellPhotosWH (circleCellWidth - circleCellMargin - 2 * circleCellPhotosMargin) / 3                                                                                   //图片的宽高
...
- (void)setChildView{
    ...
    [iconView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(iconViewClick)]];
...
-(void)setData{
    self.iconView.image = [UIImage imageNamed:ImagePath(self.momentFrames.moment.icon)];
...
- (void) iconViewClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(bookListViewdidSelectRowAtIndex)]) {
        [self.delegate bookListViewdidSelectRowAtIndex];
    }
}

...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, kScreenHeight, kScreenWidth, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, kScreenHeight - 64, kScreenWidth, kChatKeyBoardHeight);
    }
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        self.sepaView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.width, 0.5)];
        [self addSubview:self.sepaView];
...
- (void) addNotificationNoFrame
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyBoardWillChangeFrame:) name:UIKeyboardWillChangeFrameNotification object:nil];
    
}

- (void) deleteNoficationNoFrame
{
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    if (self.chatToolBar.faceSelected)
    {
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.frame = CGRectMake(0, [self getSuperViewH] - CGRectGetHeight(self.frame) + 49, kScreenWidth, CGRectGetHeight(self.frame));
    ...
    else
    {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            CGFloat chatToolBarHeight = CGRectGetHeight(self.frame) - self.morePanelHeight;
            
            CGFloat targetY = end.origin.y - chatToolBarHeight;
...
{
    self.chatToolBar.faceBtn.enabled = NO;
...
{
    if (!_chatToolBar) {
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, 2, kScreenWidth, kChatToolBarHeight)];
...
{
    if (!_OAtoolbar) {
        _OAtoolbar.backgroundColor = [UIColor redColor];
